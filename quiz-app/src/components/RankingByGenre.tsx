...
export default function RankingByGenre({ genreName, entries }: Props) {
  return (
    <div className="animate-fadeIn">
      <h1 className="text-2xl sm:text-4xl font-black text-center text-gray-800 mb-2">
        📚 ジャンル別ランキング
      </h1>
      <h2 className="text-xl sm:text-3xl font-bold text-center text-purple-500 mb-6 sm:mb-8">{genreName}</h2>
      <div className="space-y-2 sm:space-y-3">
        {entries.map((entry) => {
          ...
          return (
            ...
            >
              <div className={`flex items-center justify-center w-9 h-9 sm:w-12 sm:h-12 rounded-full text-base sm:text-xl font-black shrink-0 ${style.badge}`}>
                {rankIcon(entry.rank)}
              </div>
              <div className="flex-1 min-w-0 ml-3 sm:ml-4">
                <p className="text-base sm:text-xl font-bold text-gray-800 truncate">{entry.nickname}</p>
              </div>
              <div className="text-right shrink-0">
                <p className="text-lg sm:text-2xl font-black text-green-500">
                  {entry.genre_score}<span className="text-xs sm:text-base text-gray-400 ml-1">点</span>
                </p>
              </div>
            </div>
          );
